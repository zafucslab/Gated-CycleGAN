...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        temp_stage1 = []
        temp_stage1 = [nn.ReflectionPad2d(3),
        ...
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            temp_stage2 += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
        ...
        for i in range(n_blocks):       # add ResNet blocks
            ...
            temp_stage3 += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]

        temp_stage4 = []
        for i in range(n_downsampling):  # add upsampling layers
            mult = 2 ** (n_downsampling - i)
            temp_stage4 += [nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
        ...
        temp_stage5 = []
        temp_stage5 += [nn.ReflectionPad2d(3)]
        temp_stage5 += [nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0)]
        temp_stage5 += [nn.Tanh()]

        self.stage1 = nn.Sequential(*temp_stage1)
        self.stage2 = nn.Sequential(*temp_stage2)
        self.stage3 = nn.Sequential(*temp_stage3)

        self.stage4 = nn.Sequential(*temp_stage4)
        self.stage5 = nn.Sequential(*temp_stage5)
